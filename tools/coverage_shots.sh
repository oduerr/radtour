...
# Usage: tools/coverage_shots.sh [output-dir]   (renders the LOCAL genova/index.html; tiles need internet at render time)
set -e
R=$(cd "$(dirname "$0")/.." && pwd)
...
for s in $SPOTS; do
  ...
  url="file://$R/genova/index.html#d${day}/${c}/${z}"
  ...
  "$C" --headless=new --disable-gpu --hide-scrollbars --window-size=800,900 --virtual-time-budget=12000 --screenshot="$OUT/${name}_Handy.png" "file://$R/tools/shot.html?w=390&h=844&u=../genova/index.html%23d${day}/${c}/${z}" 2>/dev/null
  echo "$name"
done
ls -la "$OUT"
