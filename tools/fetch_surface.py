"""Surface class per track point from OSM (Overpass) -> tours/<slug>/osm_points.json (input of build_track.py).
...
EPS=["https://overpass-api.de/api/interpreter","https://overpass.openstreetmap.fr/api/interpreter"]
def q(query):
    for ep in EPS:
        for _ in range(2):
    ...
    return None
def cls(t):
    hw=t.get('highway',''); sf=t.get('surface',''); tt=t.get('tracktype','')
    if hw in ('path','footway','bridleway','steps') or t.get('sac_scale'): return 'path'
    if hw=='track': return 'track_g1' if tt=='grade1' or sf in ('asphalt','paved','concrete') else 'track'
    if sf in ('unpaved','gravel','fine_gravel','compacted','dirt','ground','grass','sand','earth','pebblestone','rock','mud','woodchips'): return 'unpaved_road'
    return 'paved'
...
for c0 in range(0,N,per):
    ...
    print(f"km {KM[c0]:6.1f}: {len(r.get('elements',[]))} ways ({len(ways)} total)",flush=True); time.sleep(1)
